...
export const CardContainer = ({
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Check if the user is hovering over or interacting with buttons, links, or inputs
    ...
    // When hovering over clickable controls, dampen the tilt by 80% so buttons stay stationary and easy to click
    const activeTilt = isOverInteractive ? maxTilt * 0.2 : maxTilt;

    const rotateY = clampedX * activeTilt;
    const rotateX = clampedY * activeTilt;

    containerRef.current.style.transform = `rotateY(${rotateY.toFixed(2)}deg) rotateX(${rotateX.toFixed(2)}deg)`;
  };
...
export const CardItem = ({
  ...
  useEffect(() => {
    ...
    const formatPx = (val: number | string) =>
      typeof val === "number"
        ? `${val}px`
        : val.endsWith("px")
          ? val
