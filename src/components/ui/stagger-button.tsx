...
const StaggerButton = React.forwardRef<HTMLButtonElement, StaggerButtonProps>(
  (
    ...
    // Extract text from text prop or children
    const extractedText = React.useMemo(() => {
      if (text) return text;
      if (typeof children === 'string') return children;
      // If child element is passed (e.g. <a><span>Text</span><icon/></a>)
      if (asChild && React.isValidElement(children)) {
        const childProps = children.props as { children?: React.ReactNode };
        if (typeof childProps.children === 'string') return childProps.children;
        const textInChild = React.Children.toArray(childProps.children).find(
          (c) =>
            typeof c === 'string' ||
            (React.isValidElement(c) && typeof (c.props as { children?: React.ReactNode }).children === 'string'),
        );
        if (typeof textInChild === 'string') return textInChild;
        if (React.isValidElement(textInChild)) {
          return String((textInChild.props as { children?: React.ReactNode }).children);
        }
      }
      return (
    ...
    }, [text, children, asChild]);

    const { safeBase, safeHover } = useProcessedChars(extractedText, hoverText);
    ...
    const shouldAnimate =
      disableStagger || prefersReducedMotion
        ? false
        : isTouchDevice
          ? isTapped
          : isHovered;
    ...
    const childCandidate = asChild
      ? (React.isValidElement(children)
          ? children
          : (React.Children.toArray(children).find(React.isValidElement) as
              | React.ReactElement<
    ...
    if (asChild && childCandidate) {
      const child = childCandidate;
      // Find non-text elements (like icons) inside child
      const childChildren = (child.props as { children?: React.ReactNode }).children;
      const childNonText = React.Children.toArray(childChildren).filter(
        (c) =>
          typeof c !== 'string' &&
          !(
            React.isValidElement(c) &&
            typeof (c.props as { children?: React.ReactNode }).children === 'string' &&
            (c.props as { children?: React.ReactNode }).children === extractedText
          ),
      );
      ...
      return React.cloneElement(child as React.ReactElement<any>, {
        ref,
        className: cn(
          variant ? buttonVariants({ variant, size }) : '',
          className,
        ...
        onTouchStart: (e: React.TouchEvent<HTMLElement>) => {
          handleTouchStart(e);
          (child.props as { onTouchStart?: (e: React.TouchEvent<HTMLElement>) => void }).onTouchStart?.(e);
        },
        children: (
          <>
            {innerAnimation}
